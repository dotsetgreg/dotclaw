...
/**
 * Get capabilities synchronously (from cache only, may return defaults)
 * Use this when async is not possible
 */
export function getModelCapabilitiesSync(model: string): ModelCapabilities {
  // Try to load from file if not in memory
  if (!capabilitiesCache) {
    capabilitiesCache = loadCapabilitiesCache();
  }
...
/**
 * Refresh model capabilities from API
 * Called periodically or on demand
 */
export async function refreshModelCapabilities(): Promise<void> {
  capabilitiesCache = null; // Invalidate cache
  await ensureCapabilities();
}

/**
 * Check if model capabilities need refresh
 */
export function shouldRefreshCapabilities(): boolean {
  if (!capabilitiesCache) return true;

  const fetchedAt = new Date(capabilitiesCache.fetched_at).getTime();
  return Date.now() - fetchedAt >= CAPABILITIES_CACHE_TTL_MS;
}
